  ...
  return (
    <main>
      ...
      <section className="relative bg-gradient-to-br from-green-50 via-white to-green-50 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16 sm:py-24">
          <div className="max-w-2xl">
            ...
            <p className="text-gray-900/60 text-lg mb-8">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noopener noreferrer"
                className="bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-900 font-medium px-8 py-3.5 rounded-full transition-colors text-sm flex items-center gap-2">
        ...
        {/* Info bar */}
        <div className="relative border-t border-gray-100 bg-gray-100/60">
          <div className="max-w-7xl mx-auto px-4 py-4 flex flex-wrap gap-6 text-sm text-gray-900/50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {offers.map(offer => (
              <Link key={offer.id} to="/ofertas"
                className="relative bg-gradient-to-br from-green-50 to-green-100 border border-green-200 hover:border-green-300 rounded-2xl p-5 overflow-hidden transition-all group">
                ...
                {offer.description && <p className="text-sm text-gray-900/60 mb-3">{offer.description}</p>}
                ...
                {offer.endDate && (
                  <p className="text-xs text-gray-400 mt-1">Hasta {new Date(offer.endDate).toLocaleDateString('es-PE')}</p>
                )}
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            ...
            {categories.map(cat => (
              <Link key={cat.id} to={`/catalogo?categoryId=${cat.id}`}
                className="shrink-0 bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-900 px-5 py-2.5 rounded-full text-sm transition-colors whitespace-nowrap">
                {cat.name} <span className="text-gray-400">({cat._count.products})</span>
              </Link>
            ))}
          </div>
        </section>
      ...
      <footer className="border-t border-gray-100 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 py-10 grid sm:grid-cols-3 gap-8 text-sm text-gray-400">
          <div>
            <div className="mb-3">
        ...
        <div className="border-t border-gray-100 px-4 py-4 text-center text-xs text-gray-900/20">
