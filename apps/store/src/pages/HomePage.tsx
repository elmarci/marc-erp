...
import {
  MapPin, Clock, Phone, ChevronRight, Zap, Milk, Croissant, Beef, Apple,
  ShoppingBasket, CupSoda, Cookie, Sparkles, LayoutGrid,
} from 'lucide-react'
...
// de categorías se sienta más de "supermercado" y menos como una lista de
// texto. Si el nombre no matchea, se usa un ícono genérico.
    ...
    <main>
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16 sm:py-24">
          <div className="max-w-2xl">
            ...
            <div className="flex flex-wrap gap-3">
              <Link to="/catalogo"
                className="bg-brand-blue-600 hover:bg-brand-blue-700 text-white font-bold px-8 py-3.5 rounded-full transition-colors text-sm">
                Ver productos
              </Link>
              <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noopener noreferrer"
                className="bg-white hover:bg-gray-50 border border-gray-200 text-gray-700 font-medium px-8 py-3.5 rounded-full transition-colors text-sm flex items-center gap-2 shadow-sm">
      ...
      {categories.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 py-8">
          <h2 className="text-xl font-bold mb-6 text-gray-900">Categorías</h2>
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-9 gap-3">
            <Link to="/catalogo"
              className="flex flex-col items-center gap-2 bg-brand-blue-600 hover:bg-brand-blue-700 text-white rounded-2xl px-3 py-4 transition-colors text-center">
              <LayoutGrid className="h-6 w-6" />
              <span className="text-xs font-bold">Todo</span>
            </Link>
            {categories.map(cat => {
              const Icon = CATEGORY_ICONS[cat.name.toLowerCase()] ?? ShoppingBasket
              return (
                <Link key={cat.id} to={`/catalogo?categoryId=${cat.id}`}
                  className="flex flex-col items-center gap-2 bg-white hover:bg-brand-blue-50 border border-gray-200 hover:border-brand-blue-300 rounded-2xl px-3 py-4 transition-colors text-center">
      ...
      <section className="max-w-7xl mx-auto px-4 py-6 pb-16">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link to="/catalogo" className="text-brand-blue-600 hover:text-brand-blue-700 text-sm flex items-center gap-1 transition-colors">
      ...
      <footer className="border-t border-gray-200 bg-white">
        <div className="max-w-7xl mx-auto px-4 py-10 grid sm:grid-cols-3 gap-8 text-sm text-gray-500">
          <div>
          ...
          <div>
            <p className="text-gray-900 font-medium mb-3">Contacto</p>
