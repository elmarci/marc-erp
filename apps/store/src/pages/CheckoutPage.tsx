...
export function CheckoutPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-md mx-auto px-4 py-20 text-center">
        <ShoppingBag className="h-16 w-16 mx-auto mb-4 text-gray-900/10" />
        <p className="text-gray-900/50 mb-6">Tu carrito está vacío</p>
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      {/* Steps bar */}
      <div className="sticky top-16 z-40 bg-gray-50/95 backdrop-blur border-b border-gray-200">
            ...
            {steps.map((s, i) => (
              <div key={s.num} className="flex items-center flex-1">
                <div className="flex flex-col items-center">
                  ...
                  <span className={`text-xs mt-1 font-medium ${step === s.num ? 'text-green-400' : step > s.num ? 'text-gray-900/50' : 'text-gray-900/20'}`}>
                    {s.label}
                  </span>
                </div>
                {i < steps.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-3 mb-4 transition-all ${step > s.num ? 'bg-green-500/40' : 'bg-gray-100'}`} />
                )}
              </div>
            ))}
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="grid md:grid-cols-5 gap-6">
          ...
          <div className="md:col-span-3">
            <Link to="/" className="flex items-center gap-2 text-gray-400 hover:text-gray-900 text-sm mb-6 transition-colors">
            ...
            {step === 1 && (
              <div className="space-y-5">
                <div>
                  ...
                  <p className="text-sm text-gray-400">Usaremos estos datos para contactarte</p>
                </div>

                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-900/70 mb-2">Nombre completo *</label>
                    <input value={form.customerName} onChange={set('customerName')}
                      placeholder="Ej: María García"
                      className="w-full bg-gray-50 border border-gray-200 hover:border-gray-300 focus:border-green-400 rounded-xl px-4 py-3.5 text-sm outline-none transition-colors" />
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input value={form.customerPhone} onChange={set('customerPhone')}
                        ...
                        className="w-full bg-gray-50 border border-gray-200 hover:border-gray-300 focus:border-green-400 rounded-xl pl-12 pr-4 py-3.5 text-sm outline-none transition-colors" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-900/70 mb-2">
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {([
                    ...
                    <button key={opt.val}
                      ...
                      <p className="text-sm text-gray-900/60">{opt.desc}</p>
                ...
                {form.deliveryType === 'DELIVERY' && (
                  <div className="bg-gray-50 border border-gray-200 rounded-2xl p-5 space-y-4">
                    <p className="text-sm font-semibold text-gray-900/70">📍 Dirección de entrega</p>
                    <div>
                      <label className="block text-sm text-gray-900/50 mb-2">Dirección completa *</label>
                    ...
                    <div>
                      <label className="block text-sm text-gray-900/50 mb-2">Distrito</label>
                      <select value={form.district} onChange={set('district')}
                        className="w-full bg-white border border-gray-200 focus:border-green-400 rounded-xl px-4 py-3 text-sm outline-none transition-colors">
                        {DISTRICTS.map(d => <option key={d} value={d}>{d}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm text-gray-900/50 mb-2">Referencia <span className="text-gray-400">(opcional)</span></label>
                ...
                <div>
                  <label className="block text-sm text-gray-900/50 mb-2">Notas adicionales <span className="text-gray-400">(opcional)</span></label>
                  <textarea value={form.notes} onChange={set('notes')} rows={2}
                    placeholder="Horario preferido de entrega, instrucciones especiales..."
                    className="w-full bg-gray-50 border border-gray-200 focus:border-green-400 rounded-xl px-4 py-3 text-sm outline-none transition-colors resize-none" />
                </div>

                <div className="flex gap-3">
                  <button onClick={() => setStep(1)}
                    className="px-6 py-4 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl font-medium transition-colors flex items-center gap-2">
            ...
            {step === 3 && (
              <div className="space-y-5">
                <div>
                  ...
                  <p className="text-sm text-gray-400">Elige tu método de pago preferido</p>
                </div>
                    ...
                    <button key={opt.val}
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-900/60 mt-0.5">{opt.desc}</p>
                        ...
                        <div className={`h-6 w-6 rounded-full border-2 flex items-center justify-center shrink-0 mt-1 ${
                          form.paymentMethod === opt.val ? 'border-white bg-white' : 'border-gray-300'
                        }`}>
                ...
                {(form.paymentMethod === 'YAPE' || form.paymentMethod === 'PLIN') && (
                  <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4 text-sm">
                    ...
                    <p className="text-gray-900/50">Confirma tu pedido → Te llegará el número de {form.paymentMethod} → Realizas la transferencia → Envianos tu captura por WhatsApp → ¡Pedido confirmado!</p>
                  </div>
                )}
          ...
          <div className="md:col-span-2">
            <div className="bg-white rounded-2xl p-5 border border-gray-100 sticky top-36">
                ...
                {items.map(item => (
                  <div key={item.product.id} className="flex gap-3">
                    {item.product.imageUrl ? (
                      ...
                      <div className="h-10 w-10 rounded-lg bg-gray-50 shrink-0 flex items-center justify-center">
                        <ShoppingBag className="h-4 w-4 text-gray-900/20" />
                      </div>
                    )}
              ...
              <div className="flex justify-between font-black text-lg border-t border-gray-200 mt-3 pt-3">
                <span>TOTAL</span>
