...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div
      ...
      className={`group relative bg-white border rounded-2xl overflow-hidden transition-all duration-200 select-none
        ${outOfStock
          ? 'opacity-60 cursor-not-allowed border-gray-100'
          : 'hover:bg-gray-100 border-gray-100 hover:border-green-500/30 cursor-pointer active:scale-95'
        }`}
    >
      {/* Image */}
      <div className="relative aspect-square overflow-hidden bg-gray-100">
        {product.imageUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <ShoppingBag className="h-10 w-10 text-gray-900/10" />
          </div>
        )}
        ...
        {outOfStock && (
          <div className="absolute inset-0 bg-white/40 flex items-center justify-center">
            <span className="bg-gray-100 text-gray-900 text-sm font-bold px-3 py-1.5 rounded-full">Agotado</span>
          </div>
        )}
