...
export function PromoCarousel({ offers }: { offers: Offer[] }) {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => setIndex(i => (i + 1) % offers.length), AUTOPLAY_MS)
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 pt-8">
      <div className="relative rounded-2xl overflow-hidden group"
          ...
          {offers.map((offer, i) => {
            ...
            return (
              <Link key={offer.id} to="/ofertas"
                className={`relative w-full shrink-0 h-56 sm:h-72 bg-gradient-to-br ${GRADIENTS[i % GRADIENTS.length]} text-white flex items-center overflow-hidden`}>
                {/* Decoración */}
                <div className="absolute -top-10 -right-10 h-48 w-48 rounded-full bg-white/10 blur-2xl" />
                <div className="absolute -bottom-16 right-24 h-56 w-56 rounded-full bg-white/5 blur-3xl" />

                <div className="relative z-10 px-6 sm:px-14 max-w-lg">
                  {offer.storeBadge && (
                    <span className="inline-block bg-white text-gray-900 text-xs font-black px-3 py-1 rounded-full mb-3">
                      {offer.storeBadge}
                    </span>
                  )}
                  ...
                  <span className="inline-block bg-white/95 text-gray-900 font-bold px-5 py-2.5 rounded-full text-sm">
                    Ver oferta
                  </span>
                </div>

                {/* Imagen de producto, si hay */}
        ...
        {offers.length > 1 && (
          <>
            <button onClick={(e) => { e.preventDefault(); go(index - 1) }} aria-label="Anterior"
              className="absolute left-3 top-1/2 -translate-y-1/2 h-9 w-9 bg-white/90 hover:bg-white rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity">
              <ChevronLeft className="h-5 w-5 text-gray-700" />
            </button>
            <button onClick={(e) => { e.preventDefault(); go(index + 1) }} aria-label="Siguiente"
              className="absolute right-3 top-1/2 -translate-y-1/2 h-9 w-9 bg-white/90 hover:bg-white rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5">
              {offers.map((_, i) => (
                <button key={i} onClick={(e) => { e.preventDefault(); go(i) }} aria-label={`Ir a promoción ${i + 1}`}
