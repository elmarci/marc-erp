...
export function CartDrawer() {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 z-50 h-full w-full max-w-md bg-zinc-950 border-l border-white/10 flex flex-col shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-white/10">
          <h2 className="text-lg font-bold flex items-center gap-2">
            <ShoppingBag className="h-5 w-5 text-green-400" />
            Tu carrito
            {items.length > 0 && <span className="text-sm text-white/40 font-normal">({items.length} items)</span>}
          </h2>
          <button onClick={closeCart} className="p-2 hover:bg-white/5 rounded-lg"><X className="h-5 w-5" /></button>
        </div>
          ...
          {items.length === 0 ? (
            ...
            <div key={item.product.id} className="flex gap-3 bg-white/5 rounded-xl p-3 border border-white/5">
              {item.product.imageUrl ? (
                <img src={item.product.imageUrl} alt={item.product.name} className="h-16 w-16 rounded-lg object-cover shrink-0 bg-zinc-800" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-white/40 mt-0.5">S/ {item.product.salePrice.toFixed(2)} c/u</p>
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="p-5 border-t border-white/10 space-y-4 bg-zinc-950">
            {/* Subtotals */}
            <div className="space-y-2 text-sm">
              {items.map(item => (
                <div key={item.product.id} className="flex justify-between text-white/40">
                  <span className="truncate mr-2">{item.product.name} ×{item.quantity}</span>
                  <span className="shrink-0">S/ {(item.product.salePrice * item.quantity).toFixed(2)}</span>
                </div>
              ))}
            </div>
            <div className="flex justify-between text-xl font-black border-t border-white/10 pt-3">
              <span>TOTAL</span>
            ...
            <button onClick={() => { clearCart(); closeCart() }} className="w-full text-white/20 hover:text-white/50 text-xs py-1">
              Vaciar carrito
            </button>
          </div>
        )}
